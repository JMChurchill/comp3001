...
//In mmm_sw_pipeline() routine, software pipelining is used to hide memory latency. In this case, we use two
//times  more  shared memory,  since  we  need  the  current tiles  and the  next ones.  When  the  current
//tiles are multiplied by each other, the next tiles are loaded from DDR to shared memory, in parallel; in 
//this way,  the  cores  do  not  remain  idle  until  the  tiles  are  fetched.  This  is  an  advanced 
//implementation  and perhaps out of the scope of this module.
